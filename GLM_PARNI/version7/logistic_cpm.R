...
logistic_cpm <- function(ALL, hyper_par, ...) {
  ...
  y <- hyper_par$y
  log_fixed_const <- hyper_par$log_fixed_const
  ...
  # print(sum(mu %in% c(0, 1)))
  # llh_v <- log_fixed_const + colSums(y*eta + n_trials*log(1-mu))
  
  llh_v <- log_fixed_const + colSums(y*eta - n_trials*log(1 + exp(eta)))
  # cat(sum(abs(llh_v - colSums(y*eta + n_trials*log(1-inv_logit(eta))))), "\n")

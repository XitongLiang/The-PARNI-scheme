# ALA about 0
weibull_ALA <- function(ALL, hyper_par, ...) {
  ...
  JgT_y <- t(J_gamma) %*% tilde_y
  # eta_hat <- inv_B %*% JgT_ty
  
  # tilde_theta <- as.vector(inv_B %*% JgT_y)
  # print(tilde_theta)
  ...
  grad_theta <- t(J_gamma) %*% tilde_y
  
  # print(log_det + d_theta*log(2*pi)/2 - weibull_log_post_beta(tilde_theta, t, J_gamma, d, k, g, inv_variances, p_z, p_gam) )
  
  ALL$approx_llh <- log_det + sum((L_invB %*% grad_theta)^2)/2 + d_theta*log(2*pi)/2 - 
    weibull_log_post_beta(rep(0, d_theta), t, J_gamma, d, k, g, inv_variances, p_z, p_gam)

...
ecoli = readRDS(paste0(dirname(getwd()), '/Data/ecoli70.rds'))
ecoli_W = bnlearn::as.igraph(ecoli)

W = as.matrix(as_adjacency_matrix(ecoli_W))
num_edges = sum(W)
...
for (b in 1:B) {
  ...
  X = as.matrix( bnlearn::rbn(ecoli, N) )
  X = scale(X)
...
write.csv(df_results,
          paste0("../Results/Ecoli70_B", B,".csv"))
